...
class BrowserQRCodeSvgWriter {
  ...
  private createSVGElement(w: number, h: number): SVGSVGElement {
    ...
    let svgElement: SVGSVGElement = document.getElementById('ZXingDebug') as SVGSVGElement;
    if (!svgElement) {
      svgElement = document.createElementNS(BrowserQRCodeSvgWriter.SVG_NS, 'svg');
    }


    // corrected: width -> w, height -> h
    svgElement.setAttributeNS(null, 'width', w.toString());
    svgElement.setAttributeNS(null, 'height', h.toString());

    // helpful for predictable scaling and sharp rendering
    svgElement.setAttributeNS(null, 'viewBox', `0 0 ${w} ${h}`);
    svgElement.setAttributeNS(null, 'xmlns', BrowserQRCodeSvgWriter.SVG_NS);
    svgElement.setAttributeNS(null, 'shape-rendering', 'crispEdges');
