...
export class ASCIIEncoder {
    ...
    encode(context) {
        // step B
        const n = EncoderUtils.determineConsecutiveDigitCount(context.getMessage(), context.pos);
        if (n >= 2) {
            ...
            const newMode = EncoderUtils.lookAheadTest(context.getMessage(), context.pos, this.getEncodingMode());
