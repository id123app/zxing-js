...
exports.MAX_LINEAR_CORNER_ANGLE_DEVIATION_DEG = exports.MIN_LINEAR_ASPECT_RATIO = exports.DEFAULT_LINEAR_FORMATS = void 0;
exports.hasValidLinearGeometry = hasValidLinearGeometry;
/**
 ...
 * accidental mutation.
 */
exports.DEFAULT_LINEAR_FORMATS = Object.freeze([
 ...
 */
var LINEAR_FORMAT_SET = new Set(exports.DEFAULT_LINEAR_FORMATS);
/**
 ...
 */
exports.MIN_LINEAR_ASPECT_RATIO = 1.5;
/**
 ...
 */
exports.MAX_LINEAR_CORNER_ANGLE_DEVIATION_DEG = 10;
/**
 ...
 */
function hasValidLinearGeometry(result) {
    if (!LINEAR_FORMAT_SET.has(result.format))
        return true;
    ...
    var topLeft = pos.topLeft, topRight = pos.topRight, bottomLeft = pos.bottomLeft;
    if (!topLeft || !topRight || !bottomLeft)
        return false;
    var wdx = topRight.x - topLeft.x;
    var wdy = topRight.y - topLeft.y;
    var width = Math.sqrt(wdx * wdx + wdy * wdy);
    var hdx = bottomLeft.x - topLeft.x;
    var hdy = bottomLeft.y - topLeft.y;
    var height = Math.sqrt(hdx * hdx + hdy * hdy);
    ...
    if (longSide / shortSide < exports.MIN_LINEAR_ASPECT_RATIO)
        return false;
    ...
    var dot = wdx * hdx + wdy * hdy;
    var cosAngle = dot / (width * height);
    // Clamp to [-1, 1] to defend against floating point drift
    var clamped = Math.max(-1, Math.min(1, cosAngle));
    var angleDeg = Math.acos(clamped) * 180 / Math.PI;
    if (Math.abs(angleDeg - 90) > exports.MAX_LINEAR_CORNER_ANGLE_DEVIATION_DEG)
        return false;
    return true;
}
